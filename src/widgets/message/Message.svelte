...
<script lang="ts">
  ...
  function renderReplyHeader(msg) {
    ...
    if (msg.message && msg.message !== "") {
      columns.push(
        `<div>${short && msg.message.length > 20 ? msg.message.substring(0, 20) + "..." : msg.message}</div>`,
      );
    }
  ...
  function getFullname(msg) {
    ...
    try {
      ...
      if (msg.fwdFrom && msg.fwdFrom.fromName) {
        ...
        let fn = "";
        if (sender.firstName) fn += sender.firstName;
        if (sender.lastName) fn += " " + sender.lastName;
        fullname = fn;
      }
    } catch (err) {
  ...
  beforeUpdate(async () => {
    ...
    try {
      ...
      if (message.fwdFrom) {
        if (
          chat.entity.id &&
          chat.entity.id.value.toString() !== user[0].id.toString()
        ) {
          forwardedPrefix = "Forwarded from ";
        }
        if (
          sender.id &&
          sender.id.toString() === user[0].id.toString() &&
          sender.id.toString() === chat.entity.id.value.toString()
        ) {
          _hasAvatar = true;
          _justifyContent = "start";
          _self = false;
...
<div
  ...
  style="background-color:{!short
    ? 'var(--themeColorLight)'
    : 'inherit'};{showFull ? 'height' : 'min-height'}:{!short
    ? '92%'
    : 'auto'};overflow:{!short ? 'scroll' : 'inherit'};justify-content:{chat
    .entity.className === 'Channel' && !chat.entity.megagroup
    ? 'start'
    : justifyContent};min-height:{hasAvatar ? '50px' : '0px'};"
>
  {#if hasAvatar && !chat.isUser}{@html DOMPurify.sanitize(avatarSrc)}{/if}
  {#if !self && chat.entity.__isSavedMessages}{@html DOMPurify.sanitize(
      avatarSrc,
    )}{/if}
  <div
    class="kai-list-view-content"
    style="margin-left:{(hasAvatar && !chat.isUser) ||
    (!self && chat.entity.__isSavedMessages)
      ? '45px'
      : '0px'};"
  >
    ...
    {#if message.message}
      <p>
        {@html DOMPurify.sanitize(
          message.message.length > 80 && short
      ...
      {#if message.replies && message.replies.replies > 0}
        <small class="reply"
          ><img
            ...
            width="10px"
            height="10px"
          />&nbsp;{message.replies.replies}&nbsp;</small
        >
      {/if}
      {#if message.forwards && message.forwards > 0}
        <small class="forwards"
          ><img
            alt="forwards"
            src="/icons/forward.svg"
            width="10px"
            height="10px"
          />&nbsp;{message.forwards}&nbsp;</small
        >
      {/if}
