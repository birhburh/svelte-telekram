...
<script lang="ts">
  ...
  async function getReaderable() {
      ...
      if (xhttp.readyState == 4 && xhttp.status == 200) {
        ...
        try {
          ...
          if (isProbablyReaderable(doc)) {
            ...
            result.content =
              `<style>img{}</style><h4 style="margin-top:0px;padding-top:0px;">${message.media.webpage.title}</h4>` +
              result.content;
            await (
              await cachedDatabase
            ).put("offlineWebpages", result.content, webpageId);
    ...
    xhttp.open(
      "GET",
      `https://api.codetabs.com/v1/proxy/?quest=${message.media.webpage.url}`,
      true,
    );
    xhttp.send();
  }
...
<div class="media-container">
  <div class="quote">
    ...
    {#if message.media.webpage.title}
      <p class="title">
        {message.media.webpage.title.length > 20
          ? message.media.webpage.title.substring(0, 20) + "..."
          : message.media.webpage.title}
      </p>
    {/if}
    {#if message.media.webpage.description}
      <p class="description">
        {message.media.webpage.description.length > 50
          ? message.media.webpage.description.substring(0, 50) + "..."
