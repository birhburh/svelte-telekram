...
<script lang="ts">
  ...
  function getCoordOffset($what, $lat, $lon, $offset) {
    ...
    var $radOff =
      $what === 0
        ? $offset / $earthRadius
        : $offset / ($earthRadius * Math.cos((Math.PI * $coord[0]) / 180));
        ...
        props: {
          ...
          onEnter: (evt, scope) => {
            ...
            if (scope.selected.title === "Open in Google Map") {
              window
                .open(
                  `https://www.google.com/maps/search/?api=1&query=${message.media.geo.lat},${message.media.geo.long}`,
                  "_blank",
                )
                .focus();
            } else if (scope.selected.title === "Open in OpenStreetMap") {
              const url = getOpenStreetMapEmbed(
