...
<script lang="ts">
        ...
        onEnter: async (evt, scope) => {
          ...
          if (scope.selected.title === "Open") {
            ...
            try {
              ...
              downloaded = false;
              downloading = -1;
              downloadedMediaEmitter.addListener(
                "message",
            ...
            try {
              ...
              if (fn == null) {
                fn =
                  new Date().getTime().toString() + "." + Mime.extension(mime);
              }
    ...
    if (evt.hash && evt.hash === fileId) {
      if (evt.done != null) {
        ...
        downloading = Math.round(
          (evt.progress.received / evt.progress.total) * 100,
        );
      } else if (evt.error) {
  ...
  beforeUpdate(async () => {
    ...
    try {
      const arrBuff = strippedPhotoToJpg(
        Buffer.from(message.media.document.thumbs[0].originalArgs.bytes),
      );
...
<div class="media-container">
  ...
  <small>
    <div>
      {#if downloading > -1}{downloading}%&nbsp;{/if}{#if !downloaded && downloading === -1}<img
          alt="download"
          src="/icons/download.svg"
          width="10px"
          height="10px"
        />&nbsp;{/if}{getDocumentName(message)}
    </div>
    <div>{size}</div>
  </small>
</div>
