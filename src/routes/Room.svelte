...
<script lang="ts">
  ...
  let messageMetadata: {
    [key: string]: { index: number; deleted: bool; callback: Function };
  } = {};
  ...
  let navOptions = {
    ...
    enterListener: async function (evt) {
      if (!ready && chat == null) return;
      if (
        location.state.entity.className === "Channel" &&
        !location.state.entity.megagroup
      ) {
        // Channel
    ...
    arrowDownListener: async (evt) => {
      try {
        if (
          ready &&
          navInstance.verticalNavIndex !==
            Object.keys(messageMetadata).length - 1
        ) {
          evt.preventDefault();
          navInstance.navigateListNav(1);
          const msg = messages[navInstance.verticalNavIndex];
  ...
  async function joinChannel() {
    try {
      ...
      const channels = await client.invoke(
        new Api.channels.GetChannels({ id: [location.state.entity.id.value] }),
      );
      await client.invoke(
        new Api.channels.JoinChannel({ channel: channels.chats[0] }),
      );
  ...
  async function leaveChannel() {
    ...
    try {
      ...
      await client.invoke(
        new Api.channels.LeaveChannel({ channel: channels.chats[0] }),
      );
    ...
    try {
      sendMessageDialog = new TextAreaDialog({
        ...
        props: {
          title: "Message",
          softKeyLeftText:
            messageEntity != null ? (edit ? "Save" : "Reply") : "Send",
          ...
          onSoftkeyLeft: async (evt, value) => {
            ...
            if (msg.length > 0) {
              ...
              if (messageEntity && edit) {
                result = await client.editMessage(chat, {
              ...
              const tmessages = await client.getMessages(chat, {
                ids: result.id,
              });
              if (tmessages.length > 0) {
        ...
        props: {
          ...
          onEnter: async (evt, scope) => {
            ...
            if (result) {
              if (
                result.className &&
                result.className === "messages.BotCallbackAnswer" &&
                result.message
              ) {
  ...
  function handleTelegramLink(url) {
    ...
    if (parsed.hostname.indexOf("t.me") > -1) {
      ...
      if (t_entity.length === 3) {
        ...
        t_entity.length === 2 ||
        parsed.hostname.indexOf("telegram.me") > -1
      ) {
  ...
  async function openRoom(value: any, scrollTo: number | null) {
    try {
      ...
      if (
        entity.id.value.toString() === location.state.entity.id.value.toString()
      )
        return;
      roomStack.push({
      ...
      const { appBar } = getAppProp();
      let name = "";
      if (entity.firstName) name = entity.firstName;
      if (entity.lastName) name += " " + entity.lastName;
      if (name === "" && entity.username) name = entity.username;
      else if (name === "" && entity.phone) name = entity.phone;
      else if (name === "") name = entity.id.value.toString();
      ...
      props: {
        ...
        onEnter: async (evt, scope) => {
          ...
          if (scope.selected.args.className === "MessageEntityMention") {
            ...
            scope.selected.args.className === "MessageEntityBotCommand"
          ) {
            try {
            ...
            ].indexOf(scope.selected.args.className) > -1
          ) {
  ...
  async function openContextMenu(msg, index) {
    ...
    try {
      ...
      if (msg.replyMarkup && msg.replyMarkup.rows) {
        ...
        if (
          [
            ...
            "ReplyKeyboardMarkup",
          ].indexOf(msg.replyMarkup.className) > -1 &&
          msg.replyMarkup.selective
        ) {
          show = false;
        }
        if (show) menu.push({ title: "Show Reply Buttons" });
      }
      ...
      if (
        (chat.entity.className === "Channel" && chat.entity.creator) ||
        (chat.entity.className === "Channel" &&
          chat.entity.megagroup &&
          !chat.entity.left) ||
        chat.entity.className === "User"
      ) {
      ...
      if (
        sender &&
        sender.id.value.toString() === user[0].id.value.toString()
      ) {
        if (
          (new Date().getTime() - new Date(msg.date * 1000).getTime() <
            172800000 ||
            chat.entity.__isSavedMessages) &&
          msg.fwdFrom == null
        ) {
      ...
      if (
        (chat.entity.className === "Channel" && !chat.entity.left) ||
        chat.entity.className === "User"
      ) {
        if (muteUntil === false) {
          ...
          onEnter: (evt, scope) => {
            ...
            setTimeout(async () => {
              if (scope.selected.title === "Entities") {
                ...
                scope.selected.title === "Show Full" &&
                msg.className === "Message"
              ) {
                showFull(msg, index);
              } else if (
                scope.selected.title === "Discussion" &&
                msg.className === "Message" &&
                msg.replies &&
                msg.replies.replies > 0
              ) {
                showReplies(msg);
                ...
                ["Archive", "Unarchive"].indexOf(scope.selected.title) > -1
              ) {
                archivingChat();
              }
            }, 200);
        ...
        if (reply) {
          ...
          replies.forEach((message) => {
            ...
            if (
              ...
              if (
                sender &&
                !(sender.username == null && sender.phone == null) &&
                sender.photo != null &&
                sender.photo.photoId
              ) {
  ...
  async function buildIndex(_messages) {
    ...
    _messages.forEach((message, index) => {
      if (message && message.id) {
        ...
        if (message.media && message.media.poll) {
          pollMetadata[message.media.poll.id.value.toString()] =
            message.id.toString();
        }
        ...
        if (
          ...
          if (
            message.sender &&
            message.sender.className === "User" &&
            cachedForwardedUsers[message.sender.id.value.toString()] == null
          ) {
            cachedForwardedUsers[message.sender.id.value.toString()] =
              message.sender;
          } else if (
            message.sender &&
            message.sender.className === "Channel" &&
            cachedForwardedChannels[message.sender.id.value.toString()] == null
          ) {
            cachedForwardedChannels[message.sender.id.value.toString()] =
              message.sender;
            ...
            if (cachedForwardedUsers[message.senderId.value.toString()]) {
              ...
              message.__sender =
                cachedForwardedUsers[message.senderId.value.toString()];
            } else if (
              cachedForwardedChannels[message.senderId.value.toString()]
            ) {
        ...
        if (message.replyTo) {
          if (messageMetadata[message.replyTo.replyToMsgId.toString()]) {
            replyIndex[message.replyTo.replyToMsgId.toString()] =
              messageMetadata[message.replyTo.replyToMsgId.toString()].index;
          } else {
    ...
    if (fetchForwardedUsers.length > 0) {
      ...
      try {
        const users = await client.invoke(
          new Api.users.GetUsers({ id: fetchForwardedUsers }),
        );
        users.forEach((u) => {
          cachedForwardedUsers[u.id.toString()] = u;
          if (
            !(u.username == null && u.phone == null) &&
            u.photo != null &&
            u.photo.photoId
          ) {
            httpTasks.push({
    ...
    forwardedUsersIndex.forEach((i) => {
      try {
        _messages[i].fwdFrom.sender =
          cachedForwardedUsers[_messages[i].fwdFrom.fromId.userId.toString()];
        if (
          !(
            _messages[i].fwdFrom.sender.username == null &&
            _messages[i].fwdFrom.sender.phone == null
          ) &&
          _messages[i].fwdFrom.sender.photo != null &&
          _messages[i].fwdFrom.sender.photo.photoId
        ) {
          _messages[i].iconRef =
            _messages[i].fwdFrom.sender.photo.photoId.toString();
    ...
    if (fetchForwardedChannels.length > 0) {
      ...
      try {
        const channels = await client.invoke(
          new Api.channels.GetChannels({ id: fetchForwardedChannels }),
        );
        channels.chats.forEach((c) => {
          cachedForwardedChannels[c.id.toString()] = c;
          if (
            !(c.username == null && c.phone == null) &&
            c.photo != null &&
            c.photo.photoId
          ) {
            httpTasks.push({
    ...
    forwardedChannelsIndex.forEach((i) => {
      try {
        _messages[i].fwdFrom.sender =
          cachedForwardedChannels[
            _messages[i].fwdFrom.fromId.channelId.toString()
          ];
        if (
          !(
            _messages[i].fwdFrom.sender.username == null &&
  ...
  async function fetchMessageCallback(id: number) {
    try {
      ...
      if (messageMetadata[id] && messages[messageMetadata[id].index]) {
        const update = await client.getMessages(chat, {
    ...
    try {
      ...
      switch (evt.className) {
        case "UpdateNotifySettings":
          const id = evt.peer.peer.channelId
            ? evt.peer.peer.channelId.value.toString()
            : evt.peer.peer.userId.value.toString();
          if (id !== chat.entity.id.value.toString()) break;
          if (evt.notifySettings.muteUntil) {
            muteUntil = new Date(
              new Date().getTime() + evt.notifySettings.muteUntil,
            );
          } else {
            muteUntil = false;
          }
        ...
        case "UpdateShortMessage":
          if (
            evt.userId &&
            evt.userId.value.toString() ===
              location.state.entity.id.value.toString()
          ) {
          ...
          for (let i in entities) {
            if (
              ...
              if (
                ...
                if (
                  !cachedForwardedUsers[evt.message.senderId.value.toString()]
                ) {
                  const _start = new Date().getTime();
                  const users = await client.invoke(
  ...
  function autoScroll() {
    setTimeout(() => {
      if (
        Object.keys(messageMetadata).length - navInstance.verticalNavIndex ===
        2
      )
        navInstance.navigateListNav(1);
    }, 500);
  ...
  function getReplyHeader(message) {
    ...
    if (
      replyIndex[message.replyTo.replyToMsgId] ||
      replyIndex[message.replyTo.replyToMsgId] === 0
    ) {
  ...
  async function fetchMessages(entity, _scrollAt) {
    ...
    try {
      ...
      if (chat.entity == null) chat.entity = entity;
      if (chat.entity) muteUntil = chat.entity.__muted || false;
      let latestMessages = [];
      let msgToscrollAt = [];
      if (_scrollAt != null) {
        msgToscrollAt = await client.getMessages(chat.entity, {
          limit: 1,
          ids: [_scrollAt],
        });
      }
      if (
        msgToscrollAt.length == 0 ||
        (msgToscrollAt.length == 1 && msgToscrollAt[0] == null)
      ) {
        latestMessages = await client.getMessages(chat.entity, { limit: 20 });
      } else {
        const minId = await client.getMessages(chat.entity, {
          limit: 10,
          minId: _scrollAt,
          reverse: true,
        });
        minId.reverse();
        const maxId = await client.getMessages(chat.entity, {
          limit: 10,
          maxId: _scrollAt,
        });
      ...
      navInstance.navigateListNav(1);
      setTimeout(() => {
        navInstance.navigateListNav(
          cursor || Object.keys(messageMetadata).length,
        );
        if (
          messages[navInstance.verticalNavIndex] &&
