...
function executeDownloadMediaTask() {
  if (downloadMediaTask.length <= 0) return;
  ...
  let bytes;
  client
    .getMessages(chats[task.chatId].entity, { limit: 1, ids: task.messageId })
    .then((msg) => {
      return client.downloadMedia(msg[0].media, {
        progressCallback: (received, total) => {
          self.postMessage({
            ...
            hash: hash,
            progress: {
              received: received.toJSNumber(),
              total: total.toJSNumber(),
            },
          });
        },
      });
...
function executeDownloadProfilePhotoTask() {
  if (
    ready === false ||
    (client.connected === false && downloadProfilePhotoTask.length > 0)
  ) {
    setTimeout(() => {
  ...
  client
    ...
    .catch((err) => {
      ...
      if (task.origin && chats[task.origin.chatId]) {
        client
          .getMessages(chats[task.origin.chatId], {
            ids: [task.origin.messageId],
          })
          .then((messages) => {
  ...
  switch (e.data.type) {
    ...
    case 0:
      ...
      if (e.data.params) {
        session.setDC(
          e.data.params.dcId,
          e.data.params.serverAddress,
          e.data.params.port,
        );
        if (e.data.params.authKey)
          session.setAuthKey(
            new telegram.AuthKey(
              e.data.params.authKey._key,
              e.data.params.authKey._hash,
            ),
            e.data.params.dcId,
          );
      }
      client = new telegram.TelegramClient(
        session,
    ...
    case 1:
      ...
      if (
        chats[e.data.params.chatId] &&
        queuedTask.indexOf(e.data.params.fileId) === -1
      ) {
        self.postMessage({ type: 1, hash: e.data.params.fileId, init: 1 });
        downloadMediaTask.push(e.data.params);
        if (downloadMediaTask.length === 1) executeDownloadMediaTask();
