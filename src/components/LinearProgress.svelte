<script lang="ts">
  ...
  function updateProgress(val) {
    if (val < min || val > max) return progress;
...
<div class="kai-linear-progress-content">
  ...
  <div class="kai-linear-progress-body">
    ...
    <div
      class="kai-linear-progress-loaded"
      style="width:{progress}%;border-radius:{progress === 100
        ? '4px'
        : '4px 0px 0px 4px'};"
    ></div>
    <div
      class="kai-linear-progress-thumb"
      style="left:calc({progress}% - 2px);visibility:{progress == 100 ||
      progress == 0
        ? 'hidden'
        : 'visible'};"
    ></div>
...
<style>
  ...
  .kai-linear-progress-content
    > .kai-linear-progress-body
    > .kai-linear-progress-track {
    width: 100%;
